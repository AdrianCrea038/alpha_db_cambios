    ...
    if (downloadBtn) {
        downloadBtn.addEventListener('click', () => {
            if (!isOnedriveConnected) {
                ...
                window.downloadFromOnedriveFile();
            }
        });
    }
}
...
async function loginToOnedrive() {
    if (!msalInstance) {
        mostrarNotificacion('❌ Configuración de OneDrive no disponible', 'error');
        return;
    }
...
async function ensureBackupFolder() {
    ...
    try {
        const response = await graphClient.api('/me/drive/root/children')
            .filter(`name eq '${ONEDRIVE_CONFIG.folderName}'`)
            .get();
        ...
        const folderData = {
            name: ONEDRIVE_CONFIG.folderName,
            folder: {},
            '@microsoft.graph.conflictBehavior': 'rename'
        };
        
        const newFolder = await graphClient.api('/me/drive/root/children')
            .post(folderData);
...
async function uploadToOnedrive(filename, content) {
    if (!graphClient) {
        mostrarNotificacion('❌ No conectado a OneDrive', 'error');
        return false;
    }
...
async function downloadFromOnedrive(filename) {
    ...
    try {
        ...
        const searchResponse = await graphClient.api(`/me/drive/items/${folderId}/children`)
            .filter(`name eq '${filename}'`)
            .get();
        
        if (!searchResponse.value || searchResponse.value.length === 0) {
            hideSyncProgress();
            mostrarNotificacion(`⚠️ Archivo ${filename} no encontrado en OneDrive`, 'info');
            return null;
        }
...
async function loadFromOnedrive() {
    ...
    try {
        ...
        const filesResponse = await graphClient.api(`/me/drive/items/${folderId}/children`)
            .filter("name endswith '.adb'")
            .orderby("lastModifiedDateTime desc")
            .get();
        
        if (!filesResponse.value || filesResponse.value.length === 0) {
            mostrarNotificacion('📭 No hay backups en OneDrive', 'info');
            return;
        }
        
        const fileList = filesResponse.value.map(f => ({
            name: f.name,
        ...
        mostrarNotificacion('❌ Error al listar backups', 'error');
    }
}
        ...
        <div class="modal" id="onedriveFileModal" style="display: flex;">
            <div class="modal-content" style="max-width: 500px;">
                <div class="modal-header">
                    ...
                    <button class="modal-close" onclick="cerrarModalOnedrive()">✕</button>
                </div>
                <div class="modal-body">
                    <p style="margin-bottom: 1rem;">Selecciona un archivo para restaurar:</p>
                    <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                        ${files.map(file => `
                            <button onclick="restaurarDesdeOnedrive('${file.name}')" 
                ...
                <div class="modal-footer">
                    <button class="modal-btn" onclick="cerrarModalOnedrive()">CANCELAR</button>
                </div>
            </div>
        </div>
    `;
...
function hideSyncProgress() {
    if (syncProgressDiv) {
        syncProgressDiv.remove();
        syncProgressDiv = null;
    }
}
...
window.uploadToOnedriveFile = () => {
    ...
    uploadToOnedrive(filename, JSON.stringify(dataToUpload, null, 2));
};
